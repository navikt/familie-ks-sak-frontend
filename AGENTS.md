# AGENTS.md — familie-ks-sak-frontend

Frontend and backend-for-frontend (BFF) for kontantstøtte (cash-for-care benefit) case handling.
React 19 + TypeScript + Vite frontend, Express BFF (`src/backend`), pnpm, deployed on Nais (GCP).
Owned by team-baks (`teamfamilie` namespace). Only outbound API dependency: `familie-ks-sak`.
...
src/frontend/
  ...
  context/                         # App-level React Context providers (Auth, Toast, Modal, FeatureToggles, ...)
  ...
  sider/                           # Feature pages by domain (Fagsak/Behandling/..., Dokumenter, Oppgavebenk, ...)
...
- **Forms with react-hook-form**: a dedicated `useXSkjema` hook owns `useForm`, alongside `enum XFeltnavn`
  ...
  this fully — treat it as the target shape for new or touched forms rather than an established universal rule.
