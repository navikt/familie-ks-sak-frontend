...
export const behandlingHandlers = [
    http.get<{ fagsakId: string }>('/familie-ks-sak/api/behandlinger/fagsak/:fagsakId', () => {
        return HttpResponse.json(byggSuksessRessurs([BehandlingTestdata.lagVisningBehandling()]));
    }),
    http.get<{ behandlingId: string }>('/familie-ks-sak/api/behandlinger/:behandlingId', ({ request }) => {
