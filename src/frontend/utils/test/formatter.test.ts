...
describe('utils/formatter', () => {
    ...
    test('sorterBarnEtterFødselsdato skal beholde rekkefølgen når begge barn mangler fødselsdato', () => {
        const førsteBarn = {
            ident: '1',
            fødselsdato: undefined,
            merket: false,
        ...
        const andreBarn = { ...førsteBarn, ident: '2' };

        const sortert = sorterBarnEtterFødselsdato([førsteBarn, andreBarn]);

        expect(sortert.map(barn => barn.ident)).toEqual(['1', '2']);
    });
});
