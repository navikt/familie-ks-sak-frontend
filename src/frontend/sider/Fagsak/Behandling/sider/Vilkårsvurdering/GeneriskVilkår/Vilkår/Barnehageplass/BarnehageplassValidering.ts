...
export const erAntallTimerGyldig = (felt: FeltState<string>, avhengigheter?: Avhengigheter): FeltState<string> => {
    if (felt.verdi !== '') {
        ...
        if (tellAntallDesimaler(felt.verdi) > 2) {
            return feil(felt, 'Antall timer med barnehageplass kan maksimalt oppgis med 2 desimaler');
        }
        return ok(felt);
    }
    if (avhengigheter?.resultat !== Resultat.OPPFYLT && avhengigheter?.utdypendeVilkårsvurdering.length <= 0) {
