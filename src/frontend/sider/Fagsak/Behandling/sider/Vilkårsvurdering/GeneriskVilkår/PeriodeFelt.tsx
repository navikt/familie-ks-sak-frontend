...
export function PeriodeFelt({ person, vilkårType, lagretPeriode, førsteLagredeFom, onEndret, children }: Props) {
    ...
    const fom = useDatepicker({
        defaultSelected: isoStringTilDateEllerUndefinedHvisUgyldigDato(lagretPeriode.fom),
        ...
        onValidate: validation => {
            ...
            if (ventendeFom.current) {
                oppdaterPeriode({ ...value, fom: dateTilIsoDatoStringEllerUndefined(ventendeFom.current.dato) });
    ...
    const tom = useDatepicker({
        defaultSelected: isoStringTilDateEllerUndefinedHvisUgyldigDato(lagretPeriode.tom),
        fromDate: tidligsteRelevanteDato,
        toDate: senesteRelevanteDato,
        onDateChange: dato => {
        ...
        onValidate: validation => {
            ...
            if (ventendeTom.current) {
                oppdaterPeriode({ ...value, tom: dateTilIsoDatoStringEllerUndefined(ventendeTom.current.dato) });
