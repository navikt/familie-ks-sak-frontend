...
import { TestProviders, render } from '../../../testutils/testrender';
...
describe('Behandlinger', () => {
    test('skal vise alle type behandlinger', async () => {
        ...
        expect(within(rad1).getByRole('cell', { name: 'Nasjonal' })).toBeInTheDocument();
    ...
    test('skal vise skeletons hvis kun kontantsøttebehandlinger laster', async () => {
        server.use(
            http.get('/familie-ks-sak/api/behandlinger/fagsak/1', async () => {
    ...
    test('skal vise skeletons hvis kun klagebehandlinger laster', async () => {
        server.use(
            http.get('/familie-ks-sak/api/fagsaker/1/hent-klagebehandlinger', async () => {
                await delay('infinite');
                return HttpResponse.json(byggSuksessRessurs([]));
            })
        );
    ...
    test('skal vise skeletons hvis kun tilbakekrevingsbehandlinger laster', async () => {
        server.use(
            http.get('/familie-ks-sak/api/tilbakekreving/fagsak/1', async () => {
                await delay('infinite');
                return HttpResponse.json(byggSuksessRessurs([]));
            })
        );
    ...
    test('skal vise alerts hvis innlasting av alle behandlingene feiler', async () => {
        ...
        await waitFor(() => {
            expect(
                screen.queryByText('Kontantstøttebehandlinger er ikke tilgjengelig for øyeblikket.')
            ).toBeInTheDocument();
            expect(
