...
import Styles from './SaksbehandlerErrorBoundary.module.css';
...
class InternSaksbehandletErrorBoundary extends Component<Props, State> {
    ...
    public componentDidCatch(error: any, info: any): void {
        ...
        if (Sentry.isEnabled()) {
            Sentry.setUser({
                username: this.props.saksbehandler.displayName,
                email: this.props.saksbehandler.email,
            });
    ...
    private visSentryDialog() {
        if (Sentry.isEnabled()) {
            Sentry.showReportDialog({
                ...
                user: {
                    name: this.props.saksbehandler.displayName,
                    email: this.props.saksbehandler.email,
                },
                labelName: 'NAVN',
...
export function SaksbehandlerErrorBoundary({ children }: PropsWithChildren) {
    const saksbehandler = useSaksbehandler();
    return (
        <InternSaksbehandletErrorBoundary saksbehandler={saksbehandler}>{children}</InternSaksbehandletErrorBoundary>
    );
}
