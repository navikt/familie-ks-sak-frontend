...
export const useInfotrygdMigrering = () => {
    ...
    const flyttBrukerTilBaSak = (ident: string) => {
        ...
        request<{ ident: string }, IMigreringResponseDto>({
            method: 'POST',
            data: { ident },
            url: `familie-ks-sak/api/migrering`,
        })
            .then(ressurs => {
            ...
            .catch((_error: AxiosError) => {
                settMigrerInfotrygdSakRessurs(
                    byggFeiletRessurs('Ukjent feil ved flytting av bruker til ks-sak')
                );
            });
    };
...
const hentInfotrygdsakerRequestConfig = (
    ...
    return {
        method: 'POST',
        data: { ident },
        url: '/familie-ks-sak/api/infotrygd/hent-infotrygdsaker-for-soker',
    };
};
